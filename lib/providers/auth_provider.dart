...
class AuthState {
  ...
  const AuthState({
    ...
    this.isInitializing = true, // Default to true for startup
    this.token,
    this.client,
    this.error,
  });
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  AuthNotifier(this._apiService, this._storageService)
    ...
    developer.log(
      '   Storage Service: ${_storageService.runtimeType}',
      name: 'Auth',
    );
  ...
  Future<void> _loadSession() async {
    // Use isInitializing for the initial load
    state = state.copyWith(isInitializing: true, isLoading: false);
    ...
    try {
      ...
      if (token != null && token.isNotEmpty && client != null) {
        ...
        state = state.copyWith(
          ...
          isInitializing: false, // Initialization done
          isLoading: false,
          token: token,
          client: client,
          clearError: true,
  ...
  Future<void> login({required String phone, required String orderCode}) async {
    ...
    try {
      ...
      if (token == null || clientData == null) {
        developer.log(
          '❌ Invalid response: missing token or client data',
          name: 'Auth',
        );
      ...
      developer.log(
        '   Storage Service type: ${_storageService.runtimeType}',
        name: 'Auth',
      );
...
final apiServiceProvider = Provider<LaapakApiService>((ref) {
  ...
  return LaapakApiService(apiKey: null, jwtToken: null, useDevelopment: false);
});
...
final authProvider = StateNotifierProvider<AuthNotifier, AuthState>((ref) {
  ...
  developer.log(
    '   ✅ API Service obtained: ${apiService.runtimeType}',
    name: 'Auth',
  );
  ...
  return storageServiceAsync.when(
    data: (storageService) {
      developer.log(
        '   ✅ Storage Service obtained: ${storageService.runtimeType}',
        name: 'Auth',
      );
...
final authenticatedApiServiceProvider = Provider<LaapakApiService?>((ref) {
  ...
  return LaapakApiService(
    ...
    onUnauthorized: () {
      developer.log('⚠️ Session expired, logging out...', name: 'Auth');
      // We need to use read here because this callback is executed asynchronously
      // and not during the provider build phase
